...
int main(int argc, char *argv[]) {
    ...
    cudaMalloc((void**)&dev_x1mat, sizeof(float) * num_points * point_dim);
    cudaMalloc((void**)&dev_x2mat, sizeof(float) * num_points * point_dim);
    ...
    out_transformation = (float*)malloc(point_dim * point_dim * sizeof(float));
    cudaMemcpy(out_transformation, dev_x1Tx2, point_dim * point_dim * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(dev_pt, x1mat, num_points * point_dim * sizeof(float), cudaMemcpyHostToDevice);

    // TODO Allocate and Initialize transformation matrix
    float * dev_trans_mat;
    cudaMalloc((void**)&dev_trans_mat, point_dim * point_dim * sizeof(float));
    cudaMemcpy(dev_trans_mat, out_transformation, point_dim * point_dim * sizeof(float), cudaMemcpyHostToDevice);
    ...
    //cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, point_dim, num_points, point_dim, &one_d, dev_trans_mat, point_dim, dev_pt, point_dim, &zero_d, dev_trans_pt, point_dim);
    ...
    /* for debugging
    float* debug_trans_pt = new float[num_points * point_dim];
    cudaMemcpy(debug_trans_pt, dev_trans_pt, num_points * point_dim * sizeof(float), cudaMemcpyDeviceToHost);
    for (int i = 0; i < std::min(num_points, 10); ++i) {
        std::cout << "Transformed vertex " << i << ": "
                << debug_trans_pt[i] << ", "
                << debug_trans_pt[num_points + i] << ", "
                << debug_trans_pt[2 * num_points + i] << std::endl;
    }
    delete[] debug_trans_pt;*/
 
    // So now dev_trans_pt has shape (4 x n)
    float * trans_pt; 

    trans_pt = (float*)malloc(num_points * point_dim * sizeof(float));
