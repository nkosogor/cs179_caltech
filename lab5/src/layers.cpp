...
void Dense::backward_pass(float learning_rate)
{
    ...
    &one, in_batch, in_size,
    grad_out_batch, out_size,
    &zero, grad_weights, in_size));
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    CUDNN_CALL(cudnnSetFilter4dDescriptor(filter_desc, CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW, n_kernels, c, kernel_size, kernel_size));
    ...
    CUDNN_CALL(cudnnSetConvolution2dDescriptor(conv_desc, 0, 0, stride, stride, 1, 1, CUDNN_CONVOLUTION, CUDNN_DATA_FLOAT));
...
void Conv2D::forward_pass()
{
    ...
    CUDNN_CALL(cudnnConvolutionForward(cudnnHandle, &one, in_shape, in_batch, filter_desc, weights, 
    conv_desc, fwd_algo.algo, workspace, workspace_size, &zero, out_shape, out_batch));
...
void Conv2D::backward_pass(float learning_rate)
{
    ...
    CUDNN_CALL(cudnnConvolutionBackwardFilter(cudnnHandle, &one, in_shape, in_batch, out_shape, grad_out_batch, 
    conv_desc, bwd_filter_algo.algo, workspace, workspace_size, &zero, filter_desc, grad_weights));
    ...
    CUDNN_CALL(cudnnConvolutionBackwardData(cudnnHandle, &one, filter_desc, weights, out_shape, grad_out_batch,
    conv_desc, bwd_data_algo.algo, workspace, workspace_size, &zero, in_shape, grad_in_batch));
...
void Pool2D::forward_pass()
{
    ...
    CUDNN_CALL(cudnnPoolingForward(cudnnHandle, pooling_desc, &one, in_shape, in_batch, &zero, out_shape, out_batch));
...
void Pool2D::backward_pass(float learning_rate)
{
    ...
    CUDNN_CALL(cudnnPoolingBackward(cudnnHandle, pooling_desc, &one, out_shape, out_batch, out_shape, grad_out_batch,
...
void SoftmaxCrossEntropy::forward_pass()
{
    ...
    cudnnSoftmaxForward(cudnnHandle, CUDNN_SOFTMAX_ACCURATE, CUDNN_SOFTMAX_MODE_INSTANCE, &one, in_shape,
    in_batch, &zero, out_shape, out_batch);
...
void SoftmaxCrossEntropy::backward_pass(float lr)
{
    ...
    cudaMemcpy(grad_in_batch, out_batch, sizeof(float) * n * c * h * w, cudaMemcpyDeviceToDevice);

    // TODO (set 5): set grad_in_batch = grad_in_batch - grad_out_batch using
    //               cublasSaxpy
    cublasSaxpy(cublasHandle, n * c * h * w, &minus_one, grad_out_batch, 1, grad_in_batch, 1);
